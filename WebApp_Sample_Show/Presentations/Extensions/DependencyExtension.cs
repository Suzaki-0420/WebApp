...
using WebApp_Sample_Show.Applications.Repositories;
using WebApp_Sample_Show.Applications.Services;
using WebApp_Sample_Show.Applications.Services.Impls;
using WebApp_Sample_Show.Infrastructures.Adapters;
using WebApp_Sample_Show.Infrastructures.Context;
using WebApp_Sample_Show.Infrastructures.Repositories;
using WebApp_Sample_Show.Presentations.Controllers;
using WebApp_Sample_Show.Presentations.ViewModels;
namespace WebApp_Sample_Show.Presentations.Extensions;
/// <summary>
/// 依存定義および依存性注入クラス
/// </summary>
public static class DependencyExtension
{
    ...
    private static void SettingPresentations(IServiceCollection services)
    {
        ...
        // 従業員表示ViewModelをドメインオブジェクト:従業員に変換するアダプターインターフェイスの実装
        services.AddScoped<EmployeeShowViewModelAdapter>();
        // TempDataへのEmployeeShowViewの保存・復元するためのクラス
        // コンストラクタを利用して明示的にDIコンテナにインスタンスを登録する
        services.AddScoped(
            provider =>
